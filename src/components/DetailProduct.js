import React, { Component } from 'react'
import axios from 'axios'


class DetailProduct extends Component {
 
    state = {
        products: {
            id: '',
            nama: '',
            harga:'',
            desc: '',
            src: ''
        }
    }

    

    componentDidMount(){

        let pro_id = this.props.match.params.product_id

        axios.get(
            'http://localhost:3003/products/' + pro_id
            
        ).then(res => {
            // console.log(res.data);
            this.setState({
                product: res.data
            })
            
        })
        
        
        // CARA 2
        
        // let pro_id = this.props.match.params.product_id
        // axios.get(
        //     'http://localhost:3003/products',
        //     {
        //         params: {
        //             id: pro_id
        //         }
        //     }
        // ).then(res => {
        //     console.log(res.data);
            
        // })

        // CARA 3
        // axios.get(
        //     'http://localhost:3003/products',
        //     {
        //         params: {
        //             id: this.props.match.params.product_id
        //         }
        //     }
        // ).then(res => {
        //     console.log(res.data);
            
        // })

    }
    
    
    render() {        
        // this.props.match.params.product_id
        // /detailproduct/:product_id -> definisi
        // /detailproduct/78
        var {nama, harga, desc, src} = this.state.products
            return (
                <div className='card-body col-3 m-5'>       
                    <img className='card-img-top' src={src}/>
                    <div className='card-body'>
                   
                        <h5 className='card-title'>Product: {nama}</h5>
                        <p className='card-text'>Description: {desc}</p>
                        <p className='card-text'>Rp. {harga}</p>
                        <button className='btn btn-outline-primary btn-block'>Detail</button>
                        <button className='btn btn-primary btn-block'>Add To Cart</button>
                    </div>
                </div>
            // <h1>Detail Product: {this.props.match.params.product_id}</h1>, untuk cek/ambil ID
        )
    }
}



export default DetailProduct


